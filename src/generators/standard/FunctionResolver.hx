...
@:await class FunctionResolverDef implements FunctionResolver {
	...
	public function new(_funcParser:UnresolvedFunctionParse, _argParser:UnresolvedFunctionArgParse,
			_retParser:UnresolvedFunctionRetParse, _descPublisher:DescriptionPublisher) {
	...
	@:async public function publish(dbConnection:data.WikiDB, page:UnresolvedFunctionPage) {
		...
		return @:await funcArgsPublishProm.inSequence()
			.next(_ -> funcRetsPublishProm.inSequence())
			.noise();
	}
	...
	function publishDescOrNull(dbConnection:data.WikiDB,
			unDesc:UnresolvedDescription):Promise<Null<data.Id<data.WikiDB.DescriptionStorage>>> {
