...
class DescriptionPublisherDef implements DescriptionPublisher {
	...
	public function publish(conn:data.WikiDB,
			descNodes:UnresolvedDescription):Promise<Id<DescriptionStorage>> {
		dbConnection = conn;
		if (descNodes.length < 1)
			return Promise.reject(new Error("Need at least one descNode to publish..."));
		return publishDescToDB(descNodes).next(descID -> {
			return validateInsertion(descNodes, descID).swap(descID);
		});
	}

	function validateInsertion(unresolvedDescArr:UnresolvedDescription,
			id:Id<DescriptionStorage>):Promise<Bool> {
		return dbConnection.DescItem.leftJoin(dbConnection.DescriptionStorage)
			...
			.next((descItemsArr) -> {
				...
				for (i in 0...descItemsArr.length) {
					...
					verify = sql.textValue == presql.textValue && sql.type == presql.type && verify;
					if (!verify) {
	...
	function giveDescItemsIDs(descItemsArr:Array<DescItem>, maxIndex:Int):Promise<Array<Id<DescItem>>> {
		lastPublishID = Std.int
			(Math.max(maxIndex + 1
				, lastPublishID +
				1)); // TODO, why does the maxIndex we get collide sometimes? trace futures how to resolve cleanly?
	...
	function assignFirstDescriptionStorage(initial:Id<DescItem>):Promise<Id<DescriptionStorage>> {
		return dbConnection.DescriptionStorage.insertOne({
			...
			.next((autoDescStoreID) -> dbConnection.DescriptionStorage.update((ds) -> [ds.id.set
				(autoDescStoreID)]
				, {
	...
	function createDescriptionStorages(descItemIDSS:Array<Id<DescItem>>) {
		return assignFirstDescriptionStorage(descItemIDSS[0]).next((autoDescStoreID) -> {
			return if (descItemIDSS.length > 1) {
				var process = descItemIDSS.slice(1)
					.map((descItemID) -> {
						id: autoDescStoreID,
						descItem: descItemID
					});
				dbConnection.DescriptionStorage.insertMany(process)
					.swap(autoDescStoreID);
			} else {
	...
	function publishDescToDB(descItemsArr:Array<DescItem>):Promise<Id<DescriptionStorage>> {
		return getMaxID().next((maxID) -> giveDescItemsIDs(descItemsArr
			, maxID).next((descItemIDS) -> createDescriptionStorages(descItemIDS)));
