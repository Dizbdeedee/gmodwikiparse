...
function isPageInternal(jq:CheerioAPI) {
	return jq.call("div.internal")
		.length > 0;
}

function isPageDeprecated(jq:CheerioAPI) {
	return jq.call("div.deprecated")
		.length > 0;
}
...
function publishOrNull(descPublisher:DescriptionPublisher, conn:data.WikiDB,
		desc:UnresolvedDescription):Promise<Null<data.Id<data.WikiDB.DescriptionStorage>>> {
